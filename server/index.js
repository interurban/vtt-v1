...
app.post('/api/session/:sessionId/map', upload.single('map'), (req, res) => {
  ...
  if (!sessions[sessionId]) sessions[sessionId] = { tokens: [], initiative: [] };
  const file = req.file;
...
app.get('/api/session/:sessionId/state', (req, res) => {
  const { sessionId } = req.params;
  const session = sessions[sessionId] || { tokens: [], initiative: [] };
  res.json(session);
});
...
io.on('connection', (socket) => {
  ...
  socket.on('initiative:next', (sessionId) => {
    const session = sessions[sessionId];
    if (!session || !Array.isArray(session.initiative)) return;
    if (session.initiative.length > 1) {
      const first = session.initiative.shift();
      session.initiative.push(first);
      io.to(sessionId).emit('initiative:update', session.initiative);
    }
  });
});
